...
int Search(Pokemens animal[], string name) {//Find the location of the name in the array in order to get other information about the array
    for (int i = 0; i < 5; i++) {            //pokemon number modification location
...
Pokemens Pokemen[20]; //Player's pokemons Database
Pokemens Pokemen_ai[10]; //AI's pokemons Database


int Level_1_input() {//Level1's AI's Pokemon
...
//pokemon Gym2's ai pokemons pool 
int Level_2_input() {
    level_2_AI_1 Muk_AI;
...
// pokemon Gym1's player's pokemons pool 
int Player_input() {
...
// pokemon Gym2's player's pokemons pool 
int level_2_addable_pig_input() { //addable pokemons when finish the first gym
...
bool lose(vector<string>bag) {//The victory condition is that all the pokemons in the ai'bag are dead
...
void fight(vector<string>bag, vector<string>bag_ai) {
    ...
    int choose_ai = (rand() % 1);  //AI selects the battling pokemon
    cout << "[System]=> " << "Opponent sent [" << bag_ai[choose_ai] << "] to against your Pokemon !" << endl;
    int enemy = Search(Pokemen_ai, bag_ai[choose_ai]);//Get the data of the pokemons selected by ai
    int fighting_Pokemen = Search(Pokemen, pname);//Get the data of your own pokemons

    while (true) {
        while (bag[name] == "") {
            int* temp = new int();
            *temp = name;
            name = (rand() % 2);//The player's pokemons must be alive
            while (name == *temp) {
                name = (rand() % 2);
            }
            delete temp;
        ...
        //        Judgment phase
        if (Pokemen[fighting_Pokemen].harmful_abnormal_state > 0) { // Determine whether the pokemon is in an abnormal state
        ...
        while (bag[name] == "") {
            int* temp = new int();
            *temp = name;
            name = (rand() % 2);//The player's pokemon must be alive
            while (name == *temp) {
                name = (rand() % 2);
            }
            delete temp;
        ...
        if (Pokemen[fighting_Pokemen].controlled_abnormal_state == 0) {
            ...
            if (attack_choose == 1) { //Calculation of damage when using normal attacks
                cout<<"\033c"; //After the selection of skills combat clear screen 
                ...
                if (Pokemen_ai[enemy].defense < Pokemen[fighting_Pokemen].attack) { 
                    Pokemen_ai[enemy].HP -= (Pokemen[fighting_Pokemen].attack - Pokemen_ai[enemy].defense);//HP deduction according to the formula
                }
                else {
            ...
            if (attack_choose == 2) { //When attacking with skills
                ...
                cout << "[System]=> [" << bag[name] << "] does [" << Pokemen[fighting_Pokemen].skill_attack - Pokemen_ai[enemy].defense << "] damage to [" << Pokemen_ai[enemy].name << "] !" << endl; 
                //cout << Pokemen_ai[enemy].name << " has been attacked!" << endl;
        ...
        if (!alive(Pokemen_ai, enemy)) { //Determine if ai pokemon is still alive
            cout << "[System]=> [" << Pokemen_ai[enemy].name << "] is out of Vitality ! " << endl;
            bag_ai[choose_ai] = "";
        }
        ...
        while (bag_ai[choose_ai] == "") {
            ...
            *temp = choose_ai;
            ...
            delete temp;
            enemy = Search(Pokemen_ai, bag_ai[choose_ai]);//Get the data of ai reselected pokemon
        }
        //  Stroke stage
        //   Determining the status of pokemon
        if (Pokemen_ai[enemy].controlled_abnormal_state == 0) {   //Determining if pokemon unable to do any action 
            ...
            if (AI_attack_choose == 0) { //  ai using normal attack
                ...
                if (Pokemen[fighting_Pokemen].defense < Pokemen_ai[enemy].attack) { //Determine if player pokemon defense is higher than AI attack power
            ...
            //   ai If ai chooses to use the skil
        ...
        //  Determining the HP level of pokemon
        if (!alive(Pokemen, fighting_Pokemen)) {
            cout << "[System]=> [" << Pokemen[fighting_Pokemen].name << "] is out of Vitality ! " << endl;
            bag[name] = "";
        }
...
int main_battle() {
    srand((unsigned)time(NULL));
    int input_num;
    int AI_input_num; // Determine how many pokemons are in the ai pokemons pool

    switch (level_count) {
        case int(1) :
            input_num = Player_input(); //Retrieve player pokemons data and read how many there are in total
            ...
            case int(2) :
                ...
                case int(3) :
                    ...
                    case int(4) :
                        ...
                        cout << "Standing on the top trainer ring, you successfully defeated the previous world champion of the pokemon Contest.\n";
                        cout << "Yes, you are now a veritable pokemon Master.Youand your pokemon deserve it all.\n";
                        cout << "\"Congratulations!\", applause, screams, and words of congratulations are constantly heard from the stands to youand your pokemon.\n";
                        cout << "You enjoy the sense of accomplishment that this moment brings immensely.\n";
                        cout << "\"Congratulations!\", the previous pokemon Master walked over to you, patted your shoulder, and said with a smile.\n";
                        cout << "Then, take off the pokemon Master Gold Badge pinned to his chest, and place it in your hand.\n";
    ...
    for (int j = 0; j < 2; j++) {//Select twice
        ...
        char backpack_choose = 'N'; // Determine if it is OK to put the pokemon in the backpack variable
        while (backpack_choose != 'Y') { //Check the properties of the pokemon and choose whether or not to place the pokemon in the battle pack
            cout<<"\033c";
            cout << "[PokeBag]: ";
            ...
            for (int i = 0; i < input_num; i++) {           //pokemon number modification location
                cout << "[" << i << "]" << " ===> " << Pokemen[i].name << endl;    
            }
            cin >> choose_num;  //Change input pokemon name to input number, easy for players to input
            name = Pokemen[choose_num].name;
            int i = Search(Pokemen, name);
            while (choose_num == *temp||i==-1) {//Avoiding Mistakes
                cout << "[PokeBag]: ";
    ...
    delete temp;
    //AI selects the battling pokemon
    int ai_1 = rand() % AI_input_num; //AI selects the battling pokemon randomly in pokemon box
    int ai_2 = rand() % AI_input_num;
    while (ai_1 == ai_2) {  //  Make sure you don't pick a duplicate
        ai_2 = rand() % AI_input_num;
    }
    Sleep(200); //defer output
    cout<<"\033c";
    cout << "[System]=> " << "Let the battle begin !" << endl;     
    Sleep(200);
    cout << "[Gym Master]: " << endl;//Two AIs and the player fight each other, and only one attack is chosen each time, which counts as one round. No abnormal states have been added yet
    ...
    rest();
    vector<string>bag_ai;//ai pokemons
